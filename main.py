...
def phase_one(all_vars, A, b, c, art_vars, row_types):
    ...
    tableau, basic_vars = simplex_iterations(tableau, basic_vars, all_vars, "Поиск допустимого базиса")

    W = tableau[-1][-1]
    if abs(W) > 1e-6:
        raise ValueError(f"Поиск допустимого базиса: допустимого решения не существует (W* = {W})")
...
def phase_two(all_vars, tableau, basic_vars, orig_c, goal):
    ...
    tableau, basic_vars = simplex_iterations(tableau, basic_vars, all_vars, "Поиск оптимального решения")
    return tableau, basic_vars
